...
class MinorUpdateModel(models.Model):
    updateTitle = models.CharField(max_length=255, default="아직 추가 패치 노트 없음")
    version = models.ForeignKey(VersionModel, on_delete=models.CASCADE)
    updateDate = models.DateTimeField(default=timezone.now())
    ...
    def __str__(self):
        return str(self.version.version) + '/ ' + str(self.updateTitle)

class MinorUpdateItemModel(models.Model):
    itemName = models.CharField(max_length=255)
    item = models.TextField()
    minorUpdate = models.ForeignKey(MinorUpdateModel, on_delete=models.CASCADE)
    updateDate = models.DateTimeField(default=timezone.now())
